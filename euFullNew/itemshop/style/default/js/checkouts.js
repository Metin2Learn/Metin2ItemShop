...
async function initialize() {
	const { id, clientSecret } = await fetch("../system/pay.card.php", {
		method: "POST",
...
async function handleSubmit(e) {
	...
	const { id, customer_id } = await fetch("../system/pay.card.php", {
		method: "POST",
		headers: { "Content-Type": "application/json" },
		...
		switch (paymentIntent.status) { 
			case "succeeded":
				...
				.then(data => {
					if (data.payment_id) {
						transErr = 0;
						window.location.href = data.payment_id;
					} else {
						window.location.href = data.payment_id;
					}
				})
