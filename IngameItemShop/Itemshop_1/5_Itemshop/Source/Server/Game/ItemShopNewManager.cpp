...
void ItemShopNewManager::Open(LPCHARACTER ch)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "Du musst noch warten!");
		return;
	}
...
void ItemShopNewManager::Buy(LPCHARACTER ch, uint16_t id, uint16_t count)
{
	...
	if (item == m_Items.end())
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "Dieses Item wurde nicht gefunden!");
		return;
	}
	...
	if (item->second->limited)
	{
		...
		{
			ch->ChatPacket(CHAT_TYPE_INFO, "Dieses Item ist abgelaufen oder ausverkauft!");
			return;
		}

		if (static_cast<long long>(item->second->limitedCount)-count < 0)
		{
			ch->ChatPacket(CHAT_TYPE_INFO, "So viele limitierte Items sind nicht mehr verfügbar!");
			return;
		}
	}
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "Du hast nicht genug Coins!");
		return;
	}
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "Du hast nicht genug Platz!");
		return;
	}

	if (!ChangeCoins(ch, item->second->coins*count, category->second->main_category))
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "Irgendwas passt mit den Coins nicht :c");
		return;
	}

	ch->AutoGiveItem(item->second->vnum, count, -1, false);
